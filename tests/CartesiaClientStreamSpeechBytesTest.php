...
class CartesiaClientStreamSpeechBytesTest extends TestCase
{
    ...
    public function testStreamSpeechBytes()
    {
        ...
        if ($useMock) {
            ...
            $mockResponse = json_decode(file_get_contents(__DIR__ . '/mock/CartesiaClientStreamSpeechBytesMock-StreamSpeechBytes.json'), true);
            ...
            file_put_contents(__DIR__ . '/mock/CartesiaClientStreamSpeechBytesMock-StreamSpeechBytes.json', json_encode($responseData, JSON_PRETTY_PRINT));
        }
    ...
    public function testStreamSpeechBytesWithInvalidData()
    {
        ...
        // Espera-se que uma exceção seja lançada para dados inválidos
        $this->expectException(\InvalidArgumentException::class);

        // Dados inválidos não passam pela validação e portanto não são enviados para a API
    ...
    {
        $this->expectException(CartesiaClientException::class);
        $this->expectExceptionCode(401);
        
        $client = new CartesiaClient('invalid_api_key');

        // Tentar chamar a API, o que deve lançar uma exceção
        $client->streamSpeechBytes([
            'context_id' => 'happy-monkeys-fly',
            'model_id' => 'sonic-multilingual',
            'transcript' => "Hello, world! I'm generating audio on Cartesia.",
